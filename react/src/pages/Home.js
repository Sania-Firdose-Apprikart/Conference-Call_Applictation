...
function Home(props) {
  ...
  const [createRoomPasswordDialogOpen, setCreateRoomPasswordDialogOpen] =
  ...
  const handleCreateMeeting = () => {
    ...
    if (
      ...
      settings.roomCreationPasswordEnabled
    ) {
  ...
  const goToLobby = React.useCallback(
    (roomId, joinToken) => {
  ...
  React.useEffect(() => {
    ...
    if (latestMessage) {
      ...
      if (obj.command === "setSettings") {
        ...
        console.log(
          "roomCreationPasswordEnabled: ",
          localSettings.roomCreationPasswordEnabled
        );
        ...
        if (obj.authenticated && obj.joinToken && obj.roomName) {
          ...
          joinRoomUrl.current =
            currentURL + obj.roomName + "?token=" + obj.joinToken;
          setRoomName(obj.roomName);
          setGoToLobbyDialogOpen(true);
        } else {
          alert(
            "Room creation password is not correct. Please set password on app configuration file and enable JWT token stream security from settings."
          );
        }
      }
    }
