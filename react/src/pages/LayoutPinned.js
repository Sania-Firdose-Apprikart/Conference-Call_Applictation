...
function LayoutPinned(props) {
  const pinnedParticipant = props.videoTrackAssignments.find(
    (e) => e.streamId === props.pinnedParticipant?.streamId
  );

  let MAX_VIDEO_AT_SIDE = 4;

  let trackCount = Math.min(
    props.globals.desiredTileCount - 1,
    MAX_VIDEO_AT_SIDE
  );
  ...
  const maxPlayingParticipantsCount = showOthers
    ? Math.max(2, trackCount)
    : Math.min(props.videoTrackAssignments.length, MAX_VIDEO_AT_SIDE);
  const playingParticipants = [];

  const pinnedVideo = () => {
    let pinnedParticipantName;
    if (pinnedParticipant !== undefined) {
      playingParticipants.push(
        props.videoTrackAssignments.find(
          (e) => e.streamId === pinnedParticipant.streamId
        )
      );
      pinnedParticipantName =
        props?.allParticipants[pinnedParticipant.streamId]?.name;
    }
      ...
      <>
        {
          ...
          props.videoTrackAssignments.map((element, index) => {
            ...
            let participantName =
              props?.allParticipants[element?.streamId]?.name;
            ...
            if (
              element?.streamId !== pinnedParticipant?.streamId &&
              playingParticipantsCount < maxPlayingParticipantsCount
            ) {
