...
const MeetingRoom = React.memo((props) => {
  ...
  const pinnedParticipant =
    props?.allParticipants[props.currentPinInfo?.streamId];

  const pinLayout =
    typeof props.currentPinInfo !== "undefined" && props.currentPinInfo?.pinned;

  /* istanbul ignore next */
  return (
    <Container id="meeting-room">
      {props?.isRecordPluginActive === true ? <RecordingButton /> : null}
      <MuteParticipantDialog
        isMuteParticipantDialogOpen={props?.isMuteParticipantDialogOpen}
        setMuteParticipantDialogOpen={(open) =>
          props?.setMuteParticipantDialogOpen(open)
        }
      ...
      <BecomePublisherConfirmationDialog
        ...
        isBecomePublisherConfirmationDialogOpen={
          props?.isBecomePublisherConfirmationDialogOpen
        }
      />
      ...
      {props?.showEmojis && (
        ...
        >
          <ReactionBarSelector
            ...
            iconSize={28}
            style={{ backgroundColor: theme.palette.themeColor?.[70] }}
            onSelect={sendEmoji}
          />
        </div>
      )}
      <Footer
        ...
        globals={props?.globals}
        handleSendNotificationEvent={props?.handleSendNotificationEvent}
      />
    </Container>
  );
});
