...
function AntMedia(props) {
  ...
  const [isMuteParticipantDialogOpen, setMuteParticipantDialogOpen] =
    React.useState(false);

  // set participant id you wanted to mute.
  const [participantIdMuted, setParticipantIdMuted] = React.useState({
    streamName: "",
    streamId: "",
  });
  ...
  const [isAdmin, setIsAdmin] = React.useState(
    admin === true ||
      role === WebinarRoles.Host ||
      role === WebinarRoles.ActiveHost
  );
  ...
  const [microphoneButtonDisabled, setMicrophoneButtonDisabled] =
    React.useState(false);
  ...
  const [appSettingsMaxVideoTrackCount, setAppSettingsMaxVideoTrackCount] =
    React.useState(6);
  ...
  const [selectedBackgroundMode, setSelectedBackgroundMode] =
    React.useState("");
  const [selectedVideoEffect, setSelectedVideoEffect] = React.useState(
    VideoEffect.NO_EFFECT
  );
  ...
  const [videoSendResolution, setVideoSendResolution] = React.useState(
    localStorage.getItem("videoSendResolution")
      ? localStorage.getItem("videoSendResolution")
      : "auto"
  );
  ...
  const [isReconnectionInProgress, setIsReconnectionInProgress] =
    React.useState(false);

  const [highResourceUsageWarningCount, setHighResourceUsageWarningCount] =
    React.useState(0);
    ...
    setTimeout(() => {
      if (speedTestProgress.current < 40) {
        ...
        setSpeedTestObjectFailed(
          "Speed test failed. It may be due to firewall, wi-fi or network restrictions. Change your network or Try again "
        );
      }
  ...
  function stopSpeedTest() {
    if (speedTestForPublishWebRtcAdaptor.current) {
      speedTestForPublishWebRtcAdaptor.current.stop(
  ...
  function speedTestForPublishWebRtcAdaptorInfoCallback(info, obj) {
    if (info === "initialized") {
      ...
      speedTestForPublishWebRtcAdaptor.current.enableStats(
        "speedTestStream" + speedTestStreamId.current
      );
  ...
  function calculateThePublishSpeedTestResult() {
    ...
    updatedStats.videoRoundTripTime = parseFloat(
      statsList.current[statsList.current.length - 1].videoRoundTripTime
    ); // we can use the last value
    updatedStats.videoRoundTripTime =
      updatedStats.videoRoundTripTime === -1
        ? 0
        : updatedStats.videoRoundTripTime;

    updatedStats.audioRoundTripTime = parseFloat(
      statsList.current[statsList.current.length - 1].audioRoundTripTime
    ); // we can use the last value
    updatedStats.audioRoundTripTime =
      updatedStats.audioRoundTripTime === -1
        ? 0
        : updatedStats.audioRoundTripTime;

    updatedStats.videoPacketsLost =
      parseInt(
        statsList.current[statsList.current.length - 1].videoPacketsLost
      ) +
      parseInt(
        statsList.current[statsList.current.length - 2].videoPacketsLost
      ) +
      parseInt(
        statsList.current[statsList.current.length - 3].videoPacketsLost
      );

    updatedStats.videoPacketsLost =
      updatedStats.videoPacketsLost < 0 ? 0 : updatedStats.videoPacketsLost;

    updatedStats.totalVideoPacketsSent =
      parseInt(
        statsList.current[statsList.current.length - 1].totalVideoPacketsSent
      ) +
      parseInt(
        statsList.current[statsList.current.length - 2].totalVideoPacketsSent
      ) +
      parseInt(
        statsList.current[statsList.current.length - 3].totalVideoPacketsSent
      );

    updatedStats.totalVideoPacketsSent =
      updatedStats.totalVideoPacketsSent < 0
        ? 0
        : updatedStats.totalVideoPacketsSent;

    updatedStats.audioPacketsLost =
      parseInt(
        statsList.current[statsList.current.length - 1].audioPacketsLost
      ) +
      parseInt(
        statsList.current[statsList.current.length - 2].audioPacketsLost
      ) +
      parseInt(
        statsList.current[statsList.current.length - 3].audioPacketsLost
      );

    updatedStats.totalAudioPacketsSent =
      parseInt(
        statsList.current[statsList.current.length - 1].totalAudioPacketsSent
      ) +
      parseInt(
        statsList.current[statsList.current.length - 2].totalAudioPacketsSent
      ) +
      parseInt(
        statsList.current[statsList.current.length - 3].totalAudioPacketsSent
      );

    updatedStats.totalAudioPacketsSent =
      updatedStats.totalAudioPacketsSent < 0
        ? 0
        : updatedStats.totalAudioPacketsSent;

    updatedStats.audioPacketsLost =
      updatedStats.audioPacketsLost < 0 ? 0 : updatedStats.audioPacketsLost;

    updatedStats.videoJitter =
      (parseFloat(statsList.current[statsList.current.length - 1].videoJitter) +
        parseFloat(
          statsList.current[statsList.current.length - 2].videoJitter
        )) /
      2.0;
    updatedStats.videoJitter =
      updatedStats.videoJitter === -1 ? 0 : updatedStats.videoJitter;

    updatedStats.audioJitter =
      (parseFloat(statsList.current[statsList.current.length - 1].audioJitter) +
        parseFloat(
          statsList.current[statsList.current.length - 2].audioJitter
        )) /
      2.0;
    updatedStats.audioJitter =
      updatedStats.audioJitter === -1 ? 0 : updatedStats.audioJitter;

    updatedStats.currentOutgoingBitrate = parseInt(
      statsList.current[statsList.current.length - 1].currentOutgoingBitrate
    ); // we can use the last value
    updatedStats.currentOutgoingBitrate =
      updatedStats.currentOutgoingBitrate === -1
        ? 0
        : updatedStats.currentOutgoingBitrate;

    let rtt = (
      (parseFloat(updatedStats.videoRoundTripTime) +
        parseFloat(updatedStats.audioRoundTripTime)) /
      2
    ).toPrecision(3);
    let packetLost =
      parseInt(updatedStats.videoPacketsLost) +
      parseInt(updatedStats.audioPacketsLost);
    let packetLostPercentage =
      ((updatedStats.videoPacketsLost + updatedStats.audioPacketsLost) /
        (updatedStats.totalVideoPacketsSent +
          updatedStats.totalAudioPacketsSent)) *
      100;
    let jitter = (
      (parseFloat(updatedStats.videoJitter) +
        parseInt(updatedStats.audioJitter)) /
      2
    ).toPrecision(3);
    let outgoingBitrate = parseInt(updatedStats.currentOutgoingBitrate);
    let bandwidth = parseInt(
      speedTestForPublishWebRtcAdaptor.current.mediaManager.bandwidth
    );
    console.log("* rtt: " + rtt);
    ...
    if (rtt >= 0.2 || packetLostPercentage >= 3.5 || jitter >= 0.2) {
      console.log(
        "-> Your connection quality is poor. You may experience interruptions"
      );
      speedTestResult.message =
        "Your connection quality is poor. You may experience interruptions";
    } else if (rtt >= 0.1 || packetLostPercentage >= 2 || jitter >= 0.08) {
      console.log(
        "-> Your connection is moderate, occasional disruptions may occur"
      );
      speedTestResult.message =
        "Your connection is moderate, occasional disruptions may occur";
  ...
  function calculateThePlaySpeedTestResult() {
    ...
    let bytesReceivedDiff =
      stats.lastBytesReceived - stats.firstBytesReceivedCount;
    ...
    let packageReceived =
      stats.inboundRtpList.find((item) =>
        item.trackIdentifier.startsWith("ARDAMSv")
      ).packetsReceived +
      stats.inboundRtpList.find((item) =>
        item.trackIdentifier.startsWith("ARDAMSa")
      ).packetsReceived;
    let oldPackageReceived =
      oldStats.inboundRtpList.find((item) =>
        item.trackIdentifier.startsWith("ARDAMSv")
      ).packetsReceived +
      oldStats.inboundRtpList.find((item) =>
        item.trackIdentifier.startsWith("ARDAMSa")
    ...
    if (publishStats.current !== null) {
      ...
      if (deltaPackageLost > 0) {
        packageLostPercentage = (
          (deltaPackageLost / parseInt(deltaPackageReceived)) *
          100
        ).toPrecision(3);
      }
    }
    ...
    let videoJitter = stats.inboundRtpList.find((item) =>
      item.trackIdentifier.startsWith("ARDAMSv")
    ).jitterBufferDelay;
    let audioJitter = stats.inboundRtpList.find((item) =>
      item.trackIdentifier.startsWith("ARDAMSa")
    ).jitterBufferDelay;
    ...
    let rtt = (
      (parseFloat(stats.videoRoundTripTime) +
        parseFloat(stats.audioRoundTripTime)) /
      2
    ).toPrecision(3);
  ...
  function speedTestForPublishWebRtcAdaptorErrorCallback(error, message) {
    ...
    setSpeedTestObjectFailed(
      "There is an error('" + error + "'). Please try again later..."
    );
    stopSpeedTest();
  }
  ...
  function speedTestForPlayWebRtcAdaptorInfoCallback(info, obj) {
    if (info === "initialized") {
      speedTestPlayStarted.current = false;
      speedTestForPlayWebRtcAdaptor.current.play(
        "speedTestSampleStream",
        "",
        "",
        [],
        "",
      ...
      setSpeedTestObjectProgress(20);
      speedTestForPlayWebRtcAdaptor.current?.enableStats(
        "speedTestSampleStream"
      );
    } else if (info === "updated_stats") {
  ...
  function checkAndUpdateVideoAudioSources() {
    ...
    for (let index = 0; index < devices.length; index++) {
      if (
        devices[index].kind === "videoinput" &&
        devices[index].deviceId === selectedDevices.videoDeviceId
      ) {
        isVideoDeviceAvailable = true;
        setCameraButtonDisabled(false);
      }
      if (
        devices[index].kind === "audioinput" &&
        devices[index].deviceId === selectedDevices.audioDeviceId
      ) {
    ...
    if (
      ...
      if (camera) {
        ...
        setCameraButtonDisabled(false);
        console.info(
          "Unable to access selected camera, switching the first available camera."
        );
        displayMessage(
          "Unable to access selected camera, switching the first available camera.",
          "white"
    ...
    if (
      ...
      if (audio) {
        ...
        setMicrophoneButtonDisabled(false);
        console.info(
          "Unable to access selected microphone, switching the first available microphone."
        );
        displayMessage(
          "Unable to access selected microphone, switching the first available microphone.",
          "white"
    ...
    try {
      if (
        webRTCAdaptor !== null &&
        currentCameraDeviceId !== selectedDevices.videoDeviceId &&
        !isNull(publishStreamId)
      ) {
        webRTCAdaptor?.switchVideoCameraCapture(
          publishStreamId,
          selectedDevices.videoDeviceId
        );
      }
      if (
        webRTCAdaptor !== null &&
        (currentAudioDeviceId !== selectedDevices.audioDeviceId ||
          selectedDevices.audioDeviceId === "default") &&
        !isNull(publishStreamId)
      ) {
        webRTCAdaptor?.switchAudioInputSource(
          publishStreamId,
          selectedDevices.audioDeviceId
        );
      }
  ...
  function checkAndUpdateVideoAudioSourcesForPublishSpeedTest() {
    ...
    const isDeviceAvailable = (deviceType, selectedDeviceId) =>
      devices.some(
        (device) =>
          device.kind === deviceType && device.deviceId === selectedDeviceId
      );

    const updateDeviceIfUnavailable = (deviceType, selectedDeviceId) => {
      if (
        !selectedDeviceId ||
        !isDeviceAvailable(deviceType, selectedDeviceId)
      ) {
        const availableDevice = devices.find(
          (device) => device.kind === deviceType
        );
    ...
    const switchDevice = (
      ...
      if (
        speedTestForPublishWebRtcAdaptor.current &&
        currentDeviceId !== newDeviceId &&
        streamId
      ) {
        speedTestForPublishWebRtcAdaptor.current[switchMethod](
          streamId,
          newDeviceId
        );
      }
  ...
  function makeParticipantPresenter(streamId) {
    ...
    if (participantsRole === WebinarRoles.Host) {
      ...
      console.error(
        "Invalid role for participant to make presenter",
        participantsRole
      );
      return;
    }
    ...
    if (!presenterButtonDisabled.includes(streamId)) {
      setPresenterButtonDisabled((presenterButtonDisabled) => [
  ...
  function updateParticipantRole(streamId, newRole) {
    ...
    setTimeout(() => {
      handleSendNotificationEvent("UPDATE_PARTICIPANT_ROLE", publishStreamId, {
        streamId: streamId,
  ...
  function joinRoom(roomName, generatedStreamId) {
    ...
    if (!isPlayOnly) {
      ...
      console.log(
        "Play only mode is active, joining the room with the generated stream id"
      );
    }
  ...
  function removeFakeParticipant() {
    ...
    let tempVideoTrackAssignments = videoTrackAssignments.filter(
      (el) => el.streamId !== "streamId_" + suffix
    );
  ...
  function handleSubtrackBroadcastObject(broadcastObject, isPaged) {
    ...
    if (metaDataStr === "" || isNull(metaDataStr)) {
      broadcastObject.metaData =
        '{"isMicMuted":false,"isCameraOn":true,"isScreenShared":false,"playOnly":false}';
    }
  ...
  function createScreenShareWebRtcAdaptor() {
    navigator.mediaDevices
      .getDisplayMedia(getMediaConstraints("screenConstraints", 20))
      .then((stream) => {
        if (
          stream !== null &&
          !isNull(stream) &&
  ...
  function startScreenSharing() {
    ...
    screenShareWebRtcAdaptor.current.publish(
  ...
  function infoCallback(info, obj) {
    if (info === "initialized") {
      ...
      subtrackList.forEach((subTrack) => {
        ...
        allParticipantsTemp[filteredBroadcastObject.streamId] =
          filteredBroadcastObject;
      });
      ...
      Object.keys(allParticipants).forEach((participantId) => {
        if (
          participantVTAByStreamId.has(participantId) &&
          !allParticipantsTemp[participantId]
        ) {
      ...
      Object.keys(allParticipants).forEach((streamId) => {
        let broadcastObject = allParticipants[streamId];
        if (
          !isNull(broadcastObject?.isFake) &&
          broadcastObject?.isFake === true
        ) {
      ...
      if (
        obj.trackId.endsWith("_presentation") &&
        isNull(allParticipants[obj.trackId])
      ) {
...
// console.log("1928 allParticipants updated");
      ...
      if (reconnecting) {
        ...
        let newLocalVideo = document.getElementById(
          isNull(publishStreamId) ? "localVideo" : publishStreamId
        );
        ...
        webRTCAdaptor.mediaManager.setVideoCameraSource(
          publishStreamId,
          webRTCAdaptor.mediaManager.mediaConstraints,
          null,
          true
        );
        publishReconnected = true;
    ...
    // }
      ...
      try {
        if (obj.type === "FILE_MESSAGE") {
          console.log("File message content:", obj.data);
          // ... your file processing code here ...
        } else {
          handleNotificationEvent(obj);
        }
      } catch (e) {
        console.error("Error processing data_received message:", e);
      }
      ...
      if (isPlayOnly && isNoSreamExist) {
        console.log(
          "Reconnection attempt for player with no stream existmfor play only mode."
        );
      } else {
  ...
  function checkConnectionQualityForPlay(obj) {
    ...
    if (playStats.current !== null) {
      ...
      // Calculate incoming bitrate (bits per second)
      let bytesReceivedDiff =
        obj.lastBytesReceived - obj.firstBytesReceivedCount;
      ...
      let rtt = (
        (parseFloat(obj.videoRoundTripTime) +
          parseFloat(obj.audioRoundTripTime)) /
        2
      ).toPrecision(3);
      ...
      if (rtt > 0.3 || frameDropRate > 5) {
        ...
        displayPoorNetworkConnectionWarning(
          "Network connection is weak. You may encounter connection drop!"
        );
      } else if (rtt > 0.2 || frameDropRate > 2.5) {
        console.warn(`rtt: ${rtt}, average frameDropRate: ${frameDropRate}`);
        displayPoorNetworkConnectionWarning(
          "Network connection is not stable. Please check your connection!"
        );
      }
    }

    let updatedPlayStats = {
      ...
      inboundRtpList: obj.inboundRtpList,
    };
  ...
  function checkConnectionQualityForPublish(obj) {
    ...
    let jitter = (
      (parseFloat(obj.videoJitter) + parseInt(obj.audioJitter)) /
      2
    ).toPrecision(3);
    //let outgoingBitrate = parseInt(obj.currentOutgoingBitrate);

    let packageLost =
      parseInt(obj.videoPacketsLost) + parseInt(obj.audioPacketsLost);
    let packageSent =
      parseInt(obj.totalVideoPacketsSent) + parseInt(obj.totalAudioPacketsSent);
    ...
    if (publishStats.current !== null) {
      ...
      if (deltaPackageLost > 0) {
        packageLostPercentage = (
          (deltaPackageLost / parseInt(deltaPackageSent)) *
          100
        ).toPrecision(3);
      }
    }

    if (rtt >= 0.3 || packageLostPercentage >= 2.5 || jitter >= 0.08) {
      //|| ((outgoingBitrate / 100) * 80) >= obj.availableOutgoingBitrate
      console.warn(
        "rtt:" +
          rtt +
          " packageLostPercentage:" +
          ...
          " jitter:" +
          jitter
      ); // + " Available Bandwidth kbps :", obj.availableOutgoingBitrate, "Outgoing Bandwidth kbps:", outgoingBitrate);
      displayPoorNetworkConnectionWarning(
        "Network connection is weak. You may encounter connection drop!"
      );
    ...
    if (error.indexOf("NotFoundError") !== -1) {
      ...
      if (!reconnecting) {
        ...
        if (streamIdInUseCounter > 3) {
          console.log(
            "This stream id is already in use. You may be logged in on another device."
          );
          setLeaveRoomWithError(
            "Streaming is already active with your username. Please check that you're not using it in another browser tab."
          );
          setLeftTheRoom(true);
          setIsJoining(false);
      ...
      setLeaveRoomWithError(
        "Firewall might be blocking your connection. Please report this."
      );
      setLeftTheRoom(true);
      setIsJoining(false);
      setIsReconnectionInProgress(false);
      ...
      setLeaveRoomWithError(
        "System is not compatible to connect. Please report this."
      );
      setLeftTheRoom(true);
      setIsJoining(false);
      setIsReconnectionInProgress(false);
  ...
  function unpinVideo(isManual) {
    ...
    if (
      isManual &&
      isNull(currentPinInfo) &&
      currentPinInfo.streamId.endsWith("_presentation")
    ) {
      let currentPinInfoTemp = currentPinInfo;
      currentPinInfoTemp.pinned = false;
      setCurrentPinInfo(currentPinInfoTemp);
    } else {
      webRTCAdaptor?.assignVideoTrack(
        currentPinInfo?.videoLabel,
        currentPinInfo?.streamId,
        false
      );
      console.log(
        currentPinInfo?.videoLabel +
          " assigment removed from " +
          currentPinInfo?.streamId
      );
  ...
  function pinVideo(streamId) {
    ...
    if (!isNull(currentPinInfo) && currentPinInfo.streamId !== streamId) {
      console.log(
        currentPinInfo?.videoLabel +
          " assigment will be removed from " +
          currentPinInfo?.streamId
      );
      unpinVideo(false);
    }
    ...
    let assigningVideoTrack = videoTrackAssignments.find(
      (el) => el.videoLabel == videoLabel
    );
    ...
    if (!isNull(assigningVideoTrack)) {
      handleNotifyPinUser(
        streamId !== publishStreamId ? streamId : publishStreamId
      );
    ...
    if (
      (assigningVideoTrack?.isMine || assigningVideoTrack?.isReserved) &&
      assigningVideoTrack.streamId === streamId
    ) {
  ...
  function getTheAssigningVideoLabel(isLocalVideo) {
    ...
    if (isLocalVideo) {
      ...
      videoLabel =
        videoTrackAssignments[videoTrackAssignmentStartIndex]?.videoLabel;
    }
  ...
  function turnOffYourCamNotification(participantId) {
    handleSendNotificationEvent("TURN_YOUR_CAM_OFF", publishStreamId, {
  ...
  function turnOnYourMicNotification(participantId) {
    handleSendNotificationEvent("TURN_YOUR_MIC_ON", publishStreamId, {
      streamId: participantId,
      senderStreamId: publishStreamId,
    });
  }
  ...
  const displayPoorNetworkConnectionWarning = (message) => {
    ...
    if (
      ...
      Date.now() - last_warning_time > 1000 * 30
    ) {
...
console.log("2593 allParticipants updated");
  ...
  function approveBecomeSpeakerRequest(streamId) {
    setRequestSpeakerList(
      requestSpeakerList.filter((item) => item !== streamId)
    );
  ...
  function handleDebugInfo(debugInfo) {
    ...
    infoText +=
      "Video Track Assignments (" + videoTrackAssignments.length + "):\n\n";
    ...
    if (notificationEvent != null && typeof notificationEvent == "object") {
      ...
      if (
        ...
        if (eventType === "CAM_TURNED_OFF") {
          ...
          updateUserStatusMetadata(false, null, eventStreamId);
        }
  ...
  function updateParticipantCountAndList(newParticipantCount) {
    if (
      ...
      INITIAL_SUBTRACK_SIZE >
        Math.min(newParticipantCount, Object.entries(pagedParticipants).length)
    ) {
  ...
  function displayRoleUpdateMessage(streamId, oldRole, newRole) {
    if (
      ...
      // console.log(
      //   "Role update message is not displayed. Admin: ",
      //   isAdmin,
      //   " Old Role: ",
      //   oldRole,
      //   " New Role: ",
    ...
    if (oldRole.includes("active") && !newRole.includes("active")) {
      showInfoSnackbarWithLatency(
        streamId + t(" is removed from the listening room")
      );
    } else if (!oldRole.includes("active") && newRole.includes("active")) {
      showInfoSnackbarWithLatency(
        streamId + t(" is added to the listening room")
      );
    }
  }
  ...
  function checkScreenSharingStatus() {
    ...
    if (!isNull(currentPinInfo)) {
      let assignedVideoTrack = videoTrackAssignments.find(
        (el) => el.videoLabel == currentPinInfo.videoLabel
      );
    ...
    broadcastObjectsArray.forEach((broadcastObject) => {
      if (
        broadcastObject?.parsedMetaData.isScreenShared === true &&
    ...
    if (!isNull(lastlySharedScreen)) {
      ...
      if (
        !isNull(currentPinInfo) &&
        currentPinInfo.pinningTime >= lastlySharedScreenTime
      ) {
        return;
      }
  ...
  const updateRoomRecordingStatus = React.useCallback(
    (isRecording) => {
      let metadata = {};
      if (isRecording) {
  ...
  function updateVideoSendResolution(isPinned) {
    ...
    if (promise !== null) {
      promise
        ?.then(() => {
          console.info(
            "Camera resolution is updated to " + videoSendResolution + " mode"
          );
          let videoTrackSettings = webRTCAdaptor?.mediaManager?.localStream
            ?.getVideoTracks()[0]
            ?.getSettings();
          console.info(
            "Video track resolution: ",
        ...
        .catch((err) => {
          ...
          console.error(
            "Camera resolution is not updated to " +
              videoSendResolution +
              " mode. Error is " +
              err
          );
  ...
  function addMeAsParticipant(publishStreamId) {
    let isParticipantExist = videoTrackAssignments.find(
      (vta) => vta.label === "localVideo"
    );
    ...
    allParticipantsTemp[publishStreamId] = {
      streamId: publishStreamId,
      name: "You",
      parsedMetaData: { isScreenShared: false },
      status: "livestream",
    };
  ...
  function handlePlayVideo(obj) {
    ...
    if (obj.track.kind === "audio") {
      ...
      if (
        isVideoLabelExists(
          newVideoTrackAssignment.videoLabel,
          videoTrackAssignments
        )
      ) {
        console.error(
          "Video label is already exist: " + newVideoTrackAssignment.videoLabel
        );
      } else {
        console.log("add vta:" + newVideoTrackAssignment.videoLabel);
        setVideoTrackAssignments((videoTrackAssignments) => [
          ...videoTrackAssignments,
          newVideoTrackAssignment,
        ]);
        setParticipantUpdated(!participantUpdated);
  ...
  function loadMoreParticipants() {
    let size = Math.min(
      globals.participantListPagination.loadingStepSize,
      participantCount - globals.participantListPagination.currentPagePosition
    );
    if (size === 0) {
    ...
    webRTCAdaptor?.getSubtracks(
      roomName,
      null,
      globals.participantListPagination.currentPagePosition,
      size
    );
  ...
  function setAndEnableVirtualBackgroundImage(imageUrl) {
    let virtualBackgroundImage = document.getElementById(
    ...
    virtualBackgroundImage.onload = () => {
      ...
      webRTCAdaptor
        ?.enableEffect(VideoEffect.VIRTUAL_BACKGROUND)
        .then(() => {
          console.log(
            "Effect: " + VideoEffect.VIRTUAL_BACKGROUND + " is enabled"
          );
        ...
        .catch((err) => {
          console.error(
            "Effect: " +
              VideoEffect.VIRTUAL_BACKGROUND +
              " is not enabled. Error is " +
              err
          );
  ...
  const showReactions = React.useCallback(
    (streamId, streamName, reactionRequest, allParticipants) => {
      let reaction = "😀";
  ...
  const sendReactions = React.useCallback(
    (reaction) => {
    ...
    if (audioListenerIntervalJob == null) {
      audioListenerIntervalJob = setInterval(() => {
        if (!isNull(webRTCAdaptor?.remotePeerConnection[publishStreamId])) {
          webRTCAdaptor?.remotePeerConnection[publishStreamId]
            .getStats(null)
            .then((stats) => {
  ...
  const localVideoCreate = React.useCallback(
    (tempLocalVideo) => {
  ...
  function checkVideoTrackHealth() {
    ...
    if (
      localStream == null ||
      webRTCAdaptor?.mediaManager?.localStream.getVideoTracks()[0].muted
    ) {
  ...
  React.useEffect(() => {
    const tempPagedParticipants = Object.fromEntries(
      Object.entries(allParticipants).filter(
        ([key, value]) => !isNull(value?.isPaged) && value.isPaged
      )
    );
  ...
  React.useEffect(() => {
    ...
    if (obj.command === "setSettings") {
      ...
      console.log(
        ...
        localSettings?.isRecordingFeatureAvailable
      );
      setIsRecordPluginInstalled(localSettings?.isRecordingFeatureAvailable);
      if (!isNull(localSettings?.maxVideoTrackCount)) {
        console.log(
          "--maxVideoTrackCountFromAppSettings: ",
          localSettings?.maxVideoTrackCount
        );
        setAppSettingsMaxVideoTrackCount(
          localSettings?.maxVideoTrackCount > 0
            ? localSettings?.maxVideoTrackCount + 1
            : 6
        );
      }
      ...
      if (definition.success) {
        ...
        displayMessage(
          "Recording cannot be started. Error is " + definition.message,
          "white"
        );
      }
      ...
      if (definition.success) {
        ...
        handleSendNotificationEvent("RECORDING_TURNED_OFF", publishStreamId);
        displayMessage(
          "Recording stopped forcefully due to error: " + definition.message,
          "white"
        );
      }
    }
        ...
        >
              ...
              <Stack alignItems="center" justify="center" alignContent="center">
                ...
                {isNoSreamExist && isPlayOnly ? (
                  ...
                  >
                    ...
                    <b>
                      {t(
                        "You will automatically join the room once it is ready."
                      )}
                    </b>
                  </span>
                ) : (
          ...
          {isReconnectionInProgress ? (
            ...
            >
              <Stack alignItems="center" justify="center" alignContent="center">
                <CircularProgress size={52} color="inherit" />
                <span
                  style={{ margin: "27px", fontSize: 18, fontWeight: "normal" }}
                >
                  {t("Reconnecting...")}
                </span>
              </Stack>
          ...
          {leftTheRoom ? (
            ...
            <WaitingRoom
              ...
              setSpeedTestObject={(speedTestObject) =>
                setSpeedTestObject(speedTestObject)
              }
              ...
              setAudioLevelListener={(listener, period) =>
                setAudioLevelListener(listener, period)
              }
            ...
            <>
              <MeetingRoom
                ...
                toggleSetNumberOfUnreadMessages={(value) =>
                  toggleSetNumberOfUnreadMessages(value)
                }
                ...
                handleParticipantListOpen={(open) =>
                  handleParticipantListOpen(open)
                }
                requestSpeakerList={requestSpeakerList}
                handlePublisherRequestListOpen={(open) =>
                  setPublisherRequestListDrawerOpen(open)
                }
                ...
                handleSetDesiredTileCount={(count) =>
                  handleSetDesiredTileCount(count)
                }
                handleBackgroundReplacement={(mode) =>
                  handleBackgroundReplacement(mode)
                ...
                setBecomePublisherConfirmationDialogOpen={(open) =>
                  setBecomePublisherConfirmationDialogOpen(open)
                }
              ...
              <ParticipantListDrawer
                ...
                makeParticipantPresenter={(streamId) =>
                  makeParticipantPresenter(streamId)
                }
                makeParticipantUndoPresenter={(streamId) =>
                  makeParticipantUndoPresenter(streamId)
                }
                participantCount={participantCount}
                isMyMicMuted={isMyMicMuted}
              ...
              <PublisherRequestListDrawer
                ...
                rejectBecomeSpeakerRequest={(streamId) =>
                  rejectBecomeSpeakerRequest(streamId)
                }
                requestSpeakerList={requestSpeakerList}
                publishStreamId={publishStreamId}
