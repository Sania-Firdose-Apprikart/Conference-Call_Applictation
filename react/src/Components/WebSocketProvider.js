...
export const WebSocketProvider = ({ children }) => {
  ...
  const [
    latestSyncAdministrativeFieldsResponse,
    setLatestSyncAdministrativeFieldsResponse,
  ] = useState(null);
  ...
  // const applicationWebSocketUrl = "wss://vidconf.apprikart.com:5443/TestApplication/websocket/application";
  ...
  useEffect(() => {
    ...
    webSocket.current.onmessage = (event) => {
      ...
      if (
        command === "syncAdministrativeFieldsResponse" &&
        !_.isEqual(latestSyncAdministrativeFieldsResponse, newMessage)
      ) {
  ...
  return (
    <WebSocketContext.Provider
      value={{ sendMessage, latestMessage, isWebSocketConnected }}
    >
      {children}
    </WebSocketContext.Provider>
  );
