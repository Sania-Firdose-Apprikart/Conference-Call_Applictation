...
function ParticipantListButton(props) {
    ...
    return (
            <Tooltip title={t('Participant List')} placement="top">
                ...
                >
                    <SvgIcon size={32} color={props?.participantListDrawerOpen ? '#fff' : '#000'} name={'participants'} />
                  {/* eslint-disable-next-line */}
                    <a style={{color: props?.participantListDrawerOpen ? '#fff' : '#000'}}>{props?.participantCount}</a>
                </CustomizedBtn>
            </Tooltip>
        );
}
